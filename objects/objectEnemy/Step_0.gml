...
var distanceToPlayer = objectPlayer.x - x;
...
if (place_meeting(x, y + yspd, objectWall)) {
    ...
    while (!place_meeting(x, y + _pixelCheck, objectWall)) {
        y += _pixelCheck;
    }
...
if (place_meeting(x, y, objectPlayer)) {
    ...
    if (objectPlayer.y < y - 16) {
        ...
        instance_destroy();  // Destroy the enemy instance
         ...
         room_restart();  // Restart the game if colliding from the side or below
    }
}
